...
def main():
    ...
    while camera_index < 3:
        ...
        if cap.isOpened():
            print(f"✅ Webcam accessed at index {camera_index}")
            break
        print(f"❌ Failed to access webcam at index {camera_index}")
        camera_index += 1
    ...
    # Periodically log behavior data (every 5 seconds)
    ...
    try:
        while True:
            ...
            # Process frame in background (if previous processing is done)
            ...
            # Get current state data for visualization (thread-safe)
            with lock:
                ...
                # For phone visualization
                if hasattr(state, 'phone_box') and state.phone_box:
                    phone_box = state.phone_box
                    phone_confidence = getattr(state, 'phone_confidence', 0)
                else:
                    phone_box = None
            ...
            if phone_detected and phone_box:
                try:
                    ...
                    # Scale phone box coordinates to match resized frame
                    ...
                    label = f"Phone ({phone_confidence:.2f})"
                    draw_box(frame, label, (x1, y1, x2, y2), (0, 0, 255))
                except Exception as e:
                    print(f"Error drawing phone box: {e}")
        ...
        if cap:
            cap.release()
        if pose and MEDIAPIPE_AVAILABLE:
            pose.close()
